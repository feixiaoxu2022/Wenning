...
class VoiceCloneMiniMax(BaseAtomicTool):
    ...
    def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        # 参数验证
        if not audio_file:
            raise RuntimeError("audio_file不能为空")
        if not voice_id:
            raise RuntimeError("voice_id不能为空")
        if not conv_id:
        ...
        if file_ext not in ['.mp3', '.m4a', '.wav']:
            raise RuntimeError(f"不支持的音频格式: {file_ext}，仅支持 mp3/m4a/wav")

        file_size = audio_path.stat().st_size
        if file_size > 20 * 1024 * 1024:  # 20MB
            raise RuntimeError(f"文件过大: {file_size / 1024 / 1024:.1f}MB，最大支持20MB")
