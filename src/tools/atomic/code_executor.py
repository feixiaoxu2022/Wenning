...
class CodeExecutor(BaseAtomicTool):
    ...
    description = (
        ...
        "\n- 系统已安装中文字体：'WenQuanYi Zen Hei'（文泉驿正黑）、'Noto Sans CJK SC'（思源黑体）"
        "\n- 生成包含中文的图表时，必须在代码开头添加字体配置："
        "\n  import matplotlib.pyplot as plt"
        "\n  plt.rcParams['font.sans-serif'] = ['WenQuanYi Zen Hei', 'Noto Sans CJK SC', 'DejaVu Sans']"
        "\n  plt.rcParams['axes.unicode_minus'] = False  # 解决负号显示问题"
        "\n- 配置后中文将正常显示，否则会出现方框乱码"
        "\n\n【重要】PowerPoint生成最佳实践（python-pptx）："
        "\n环境：已安装python-pptx>=1.0.0，可直接import使用"
        "\n核心API："
        "\n  from pptx import Presentation"
        "\n  from pptx.util import Inches, Pt"
        "\n  prs = Presentation()  # 新建演示文稿"
        "\n  # 版式索引：0=Title Slide(封面), 1=Title and Content(标题+内容), 5=Blank(空白)"
        ...
        "\n  p = tf.add_paragraph()"
        "\n  p.text = '要点内容'"
        "\n  p.level = 0  # 0=一级要点，1=二级要点"
        "\n  prs.save('output.pptx')"
        "\n排版守则（确保专业性）："
        "\n- 每页3-5个要点（避免信息过载）"
        "\n- 标题简洁（5-10字）"
        "\n- 每个要点15-30字（过长拆分为多个要点或分页）"
        "\n- 使用层级列表（一级要点+二级细节）"
        "\n中文字体配置（避免乱码）："
        ...
        "\n                      run.font.name = 'WenQuanYi Zen Hei'  # 或'Noto Sans CJK SC'"
        "\n                      run.font.size = Pt(18)  # 正文18pt"
        "\n图片插入："
        "\n  from pptx.util import Inches"
        "\n  slide.shapes.add_picture('image.png', Inches(1), Inches(2), width=Inches(5))"
        "\n注意事项："
        "\n- 输出文件名必须以.pptx结尾"
        "\n- 生成的PPTX可在PowerPoint/WPS中完全编辑"
        "\n- 复杂图表建议先生成PNG再插入（保持清晰度）"
